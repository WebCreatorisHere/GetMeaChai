...
const navbar = () => {
  ...
  return (
    <nav className='bg-gray-900 flex justify-between text-white px-10 items-center max-md:pb-4 max-md:flex-col md:h-16'>
      <Link href={"/"}><div className="logo text-xl font-bold flex justify-center items-center"><img className='mb-[1.15rem] w-16' src="tea.gif" alt="" /><span>GetMeaChai!</span></div></Link>
      ...
      <div className=' space-x-3 relative'>
        ...
        {session &&
          <button onClick={() => { signOut() }} className='text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center media-2-out'>Logout</button>
        }
